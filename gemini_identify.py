...
C (Basket or Mouth — two variants):
- VARIANT 1: Same as K — a BOWL or CUP shape, curved bottom, small circle at right end, wider than tall
- VARIANT 2: A LENS or EYE shape — pointed at both ends, wider than tall, like R/mouth
- C and K look almost identical — both are bowl/basket shapes
- C and R look almost identical — both are lens/eye shapes
- Key: BOWL shape wider than tall OR LENS shape pointed both ends
...
E (Two Reeds or Double Strokes):
- TWO diagonal strokes or reed shapes side by side
- Can look like two parallel diagonal lines leaning
- Similar to Y but the strokes may be simpler — just lines not full leaf shapes
- OR looks like two small vertical/diagonal parallel lines
- Key: TWO parallel diagonal strokes or lines side by side
...
L (Lion):
- A LION lying down FLAT and HORIZONTAL
- Has a clear ANIMAL HEAD on the LEFT side with face/mane visible
- Body is LOW and LONG stretching to the RIGHT
- Front paws may be visible extending forward
- Tail may curl up at the RIGHT end
- Much WIDER than tall — very elongated horizontal animal shape
- Key: LION or large animal lying FLAT + horizontal + head on LEFT + long body going RIGHT
...
V (Horned Viper variant):
- Similar to F — a snake or viper shape
- May appear as a simple CURVED or WAVY line without the X head
- Or a single wavy stroke lying flat horizontally
- Softer and simpler than F — no pronounced X at the end
- Key: simple wavy or curved horizontal stroke + softer than F + no X head
...
X (Basket + Reed combined):
- Looks like a combination of K (basket/bowl) and I (reed/leaf)
- A BOWL shape with a VERTICAL STROKE or REED attached to it
- The bowl faces one direction and a vertical element rises from it
- Key: BOWL shape + VERTICAL element attached + combination of two glyphs

Z (Bolt/Door bolt horizontal):
- A short HORIZONTAL FLAT stroke or bar
- Like a flat door bolt lying horizontally
- Simple flat horizontal line or rectangle
- Much simpler than N — no zigzag peaks, just flat
- Key: FLAT HORIZONTAL stroke or bar + simple + no peaks

FINAL DECISION RULES:
STEP 1 — Check the OVERALL SHAPE first:
  Is it a BIRD? → A W or U
  Is it a SNAKE or WORM? → F V or J
  Is it a LARGE ANIMAL lying flat? → L (lion)
...
STEP 7 — C vs K vs R disambiguation:
  If it is a bowl/cup shape wider than tall → K (or C — they look the same)
  If it is a lens/eye shape pointed both ends → R (or C — they look the same)
  Use context from surrounding glyphs to decide between C and K or C and R
"""
